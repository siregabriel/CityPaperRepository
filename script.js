// Communities organized by state with sample files
const communitiesData = {
    Georgia: [
        {
            id: 1,
            name: "Madison Heights Enterprise",
            location: "Enterprise",
            files: [
        ...
        {
            id: 4,
            name: "The Goldton at Jones Farm",
            location: "Atlanta",
            files: [
        ...
        {
            id: 6,
            name: "Legacy at Savannah Quarters",
            location: "Pooler",
            files: [
        ...
        {
            id: 7,
            name: "Legacy Ridge at Alpharetta",
            location: "Alpharetta",
            files: [
    ...
    Florida: [
        ...
        {
            id: 19,
            name: "The Goldton at St. Petersburg",
        ...
        {
            id: 21,
            name: "The Canopy At Beacon Woods",
    ...
    "South Carolina": [
        ...
        {
            id: 27,
            name: "Legacy Reserve Fairview Park",
    ...
    Tennessee: [
        {
            id: 29,
            name: "The Goldton at Spring Hill",
...
const STORAGE_KEY = 'communitiesData_files';

function saveToStorage() {
    const snapshot = {};
    Object.entries(communitiesData).forEach(([state, communities]) => {
        snapshot[state] = communities.map(c => ({
            id: c.id,
            files: c.files
        }));
    });
    ...
    try {
        ...
        Object.entries(snapshot).forEach(([state, savedComs]) => {
            ...
            savedComs.forEach(savedCom => {
                ...
                if (live) live.files = savedCom.files;
            });
        });
    } catch (e) {
...
// Get all files from all communities
...
// Get all communities count
function getAllCommunitiesCount() {
    let count = 0;
    Object.values(communitiesData).forEach(stateCommunities => {
        count += stateCommunities.length;
    });
    return count;
}

// Render communities by state
function renderCommunities() {
    ...
    Object.entries(communitiesData).forEach(([state, communities]) => {
        const filteredCommunitiesInState = communities.filter(community => {
            if (searchLower === '') return true;
            
            const communityMatches = community.name.toLowerCase().includes(searchLower) || 
                                    community.location.toLowerCase().includes(searchLower);
            const fileMatches = community.files.some(file => 
                file.name.toLowerCase().includes(searchLower)
            );
            return communityMatches || fileMatches;
        });
        
        if (filteredCommunitiesInState.length > 0) {
            hasResults = true;
            ...
            stateHeader.innerHTML = `<h2 class="text-2xl font-light text-gray-700 tracking-tight">${state} <span class="text-gray-400 text-lg font-light">(${filteredCommunitiesInState.length})</span></h2>`;
            stateSection.appendChild(stateHeader);
            
            const communitiesGrid = document.createElement('div');
            communitiesGrid.className = 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-12';
            
            filteredCommunitiesInState.forEach(community => {
                const folderCard = createFolderCard(community);
                communitiesGrid.appendChild(folderCard);
            });
            
            stateSection.appendChild(communitiesGrid);
...
// Open modal with community files
...
// Download file (simulated or direct S3)
function downloadFile(fileId) {
    ...
    if (fileUrl) {
        // If it has a direct S3 URL, open it
...
function addQuickLinks() {
    ...
    if (urls.length === 0) {
        alert('Please paste at least one S3 URL');
        return;
    }
    ...
    // Parse selection: "state|communityId"
    ...
    let nextId = 1000;
    Object.values(communitiesData).forEach(stateComs => {
        stateComs.forEach(com => {
            com.files.forEach(f => {
    ...
    urls.forEach(url => {
        ...
        if (url) {
            try {
                ...
                // Add directly to community
                community.files.push(fileObj);
                addedCount++;
            } catch (e) {
...
function captureURL() {
    var url = window.location.href; // This gets current URL
    
    let urls = JSON.parse(localStorage.getItem('capturedUrl')) || []; 
    if(!urls.includes(url)) {
        urls.push(url);
        localStorage.setItem('capturedUrl', JSON.stringify(urls)); 
    }
}
// Get stored URLs function
function getStoredUrls() {
    let urls = JSON.parse(localStorage.getItem('capturedUrl')) || []; 
    console.log(urls); // You can log it or return based on your requirement
}
